...
self.used = true";
	...
	[8] = {
		...
		["enabled"] = true;
		["eventType"] = 3;
		["execute"] = "if Player.job ~= 30 or data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.NoOpener() then\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
...
\
-- if target already has Feint, skip\
if HasBuff(eventArgs.entityID, 1195) then\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
...
return nil";
	...
	[11] = {
		...
		["enabled"] = true;
		["eventType"] = 1;
		["execute"] = "if Player.job ~= 30 or Player.alive or data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.NoOpener() then\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
\
\
if data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.IsDoingMudra() == true or Player.incombat == false then \
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
...
if hasRegen and Player.hp.percent < 20 and availableSecondWind then\
		-- if sallynin installed, use hotbar, otherwise use base\
		if SallyNIN ~= nil then SallyNIN.HotBarConfig.SecondWind.enabled = false else	actionSecondWind:Cast(Player.id) end \
\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
...
if hasRegen and Player.hp.percent < 20 and availableSecondWind == false and availableBloodbath then\
		if SallyNIN ~= nil then SallyNIN.HotBarConfig.Bloodbath.enabled = false else	actionBloodbath:Cast(Player.id) end \
\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
...
";
	...
	[14] = {
		...
		["enabled"] = true;
		["eventType"] = 1;
		["execute"] = "if Player.job ~= 30 or Player.level < 80 or Player.incombat == false or data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.NoOpener() then\
...
";
	...
	[19] = {
		...
		["enabled"] = true;
		["eventType"] = 1;
		["execute"] = "if Player.job ~= 30 or Player.incombat == false or data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.NoOpener() then\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
...
\
if Player.job ~= 30 or Player.level < 80 or data.nilDataLoaded == nil or data.nilsPlayground.CustomConditionChecks.NoOpener() then\
		self.eventConditionMismatch = true -- suppressing the log\
		self.used = true \
		return nil\
end\
